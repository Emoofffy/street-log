...
| 檔案 | 職責 |
|---|---|
| `index.html` | 外殼＋底部 8 分頁導覽；內容全由 app.js 注入 `#view` |
| `app.js` | 全部邏輯；一分頁一節（見下表），`grep '===='` 得節索引 |
...
| 節 | 現況能力一句 |
|---|---|
| 資料層 | schema 權威＝`seedDB()`；`loadDB()` 負責舊資料相容（含已下架的 timeline） |
| 路由 | `go(tab)` 切分頁，查 `RENDERERS` 表呼叫各節的 render |
| 首頁 Journal | 日期＋三張入口卡（Sadhana／Physics／Journal，內容待接資料）；⚙️ 進設定 |
...
| 行事曆 Calendar | Apple 行事曆式日檢視，右側練習目標拖進時段排程 |
